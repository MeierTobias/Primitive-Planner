...
def generate_launch_file(
    ...
    with open(output_file, "w") as file:
        file.write("<launch>\n\n")
        file.write(
            '    <arg name="map_size_x" value="{}"/>\n'.format((radius + 10.0) * 2)
        )
        file.write(
            '    <arg name="map_size_y" value="{}"/>\n'.format((radius + 10.0) * 2)
        )
        ...
        file.write(
            '        <param name="map/circle_num" value="{}"/>\n'.format(
                obs_map_width * obs_map_width * obs_density / 2.0
            )
        )
        ...
        for i in range(start_num, end_num + 1):
            ...
            file.write(
                '        <arg name="target0_x"   value="{}"/>\n'.format(goal_center[0])
            )
            file.write(
                '        <arg name="target0_y"   value="{}"/>\n'.format(goal_center[1])
            )
            file.write(
                '        <arg name="target0_z"   value="{}"/>\n'.format(goal_center[2])
            )
            file.write(
                '        <arg name="goal_radius" value="{}"/>\n'.format(goal_radius)
            )
