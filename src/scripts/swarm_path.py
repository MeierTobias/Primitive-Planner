...
def generate_path(max_vel, max_acc, arc_length, arc_radius):
    ...
    if math.isinf(arc_radius):
        ...
        ss = np.linspace(0, 1, N_sample)
        theta_end = (
            radius_theta(arc_length, arc_radius)
            if radius_theta(arc_length, arc_radius) > 0
            else 0
        )
...
def solveHorizonToppAndSample(radius, ss, way_pts, vlims, alims, v_start, v_end):
    ...
    pc_acc = constraint.JointAccelerationConstraint(
        alims, discretization_scheme=constraint.DiscretizationType.Interpolation
    )
    ...
    if math.isinf(radius):
        ...
        pc_vel_norm = constraint.SecondOrderConstraint(
            inv_dyn, constraintF, constraintg, dof=2
        )
        instance = algo.TOPPRA(
            [pc_vel, pc_acc, pc_vel_norm],
            path,
            gridpoints=gridpoints,
            parametrizer="ParametrizeSpline",
        )
    ...
    if jnt_traj == None:
        raise Exception(
            "radius={} and v_start={} solve TOPP infeasible!!!".format(radius, v_start)
        )
...
def filePathEnd(lib_dir, primitive_end_all):
    ...
    for i in range(primitive_end_all.shape[0]):
        print(
            "%f" % primitive_end_all[i, 0],
            "%f" % primitive_end_all[i, 1],
            "%f" % primitive_end_all[i, 2],
            "%d" % primitive_end_all[i, 3],
            file=file_path_end,
        )
    file_path_end.close()


def obsCorrespondence(
    lib_dir, box_x, box_y, box_z, pos_all, obs_inflate_radius, voxelSize
):
    voxel_x = box_x
    voxel_y = box_y / 2
    voxel_z = box_z / 2
    ...
    file_correspondence = open(
        lib_dir + "obs_correspondence/obs_correspondence.txt", "wb+"
    )
...
def agentCorrespondence(
    lib_dir, box_x, box_y, box_z, pos_all, agent_inflate_radius, voxelSize, v_start
):
    global script_dir
    voxel_x = box_x
    voxel_y = box_y / 2
    ...
    file_correspondence = open(
        lib_dir + f"agent_correspondence/{int(round(v_start*10))}_correspondence.txt",
        "wb+",
    )
    for i in range(voxelPointNum):
        # struct.pack 转换为二进制数据
...
def timeSpaceCorrespondence(
    ...
    while v_start < max_vel + 0.05:
        ...
        filename = (
            lib_dir
            + f"trajectory/{int(round(v_start*10))}/{int(round(v_start*10))}_infeasible_id.ply"
        )
        ...
        for path_type in range(0, len(arc_radius)):
            ss, way_pts, vlims, alims = generate_path(
                max_vel, max_acc, arc_length, arc_radius[path_type]
            )
            try:
                pos_xyz, vel_xyz, acc_xyz, traj_duration = solveHorizonToppAndSample(
                    arc_radius[path_type], ss, way_pts, vlims, alims, v_start, v_end
                )
            ...
            if math.isinf(arc_radius[path_type]):
                fileVelPathID(
                    lib_dir, v_start, path_id, pos_xyz, vel_xyz, acc_xyz, traj_duration
                )
                filePosPathID(lib_dir, v_start, path_id, pos_xyz, traj_duration)

                # add path_id time
                pos_add_id_time = np.c_[
                    pos_xyz,
                    path_id * np.ones(pos_xyz.shape[0], dtype=int),
                    np.arange(0, pos_xyz.shape[0] * 10, 10),
                ]
                ...
                for theta in range(-sub_angle * 10, 360 - sub_angle * 10, delta_angle):
                    ...
                    pos_add_id_time = np.c_[
                        rot_pos_xyz,
                        path_id * np.ones(rot_pos_xyz.shape[0], dtype=int),
                        np.arange(0, rot_pos_xyz.shape[0] * 10, 10),
                    ]
                    # print("pos_add_id_time=", pos_add_id_time)
                    pos_all = np.r_[pos_all, pos_add_id_time]

                    # if abs(v_start - max_vel) < 1e-3:
                    primitive_end_add_id = np.c_[
                        rot_pos_xyz[-1, :].reshape(1, 3), path_id
                    ]
                    primitive_end_all = np.r_[primitive_end_all, primitive_end_add_id]
        ...
        if v_start == 0.0:
            ...
            if plot:
                ...
                max_val = max(
                    abs(np.max(pos_all[:, 0]) / 2),
                    abs(np.max(pos_all[:, 1])),
                    abs(np.min(pos_all[:, 1])),
                    abs(np.max(pos_all[:, 2])),
                    abs(np.min(pos_all[:, 2])),
                )
