...
def main():
    ...
    for _ in range(num_executions):
        ...
        gen_start_goal_process = start_process(
            "gen_start_goal.py", "python3 gen_start_goal.py " + str(num_drones)
        )

        # Wait for 5 seconds before starting the other two scripts
        time.sleep(1)
        gen_ego_planner_launch_process = start_process(
            "gen_primitive_planner_launch.py", "python3 gen_primitive_planner_launch.py"
        )

        time.sleep(1)
        run_ego_planner_process = start_process(
            "primitive_swarm.launch",
            "roslaunch primitive_planner primitive_swarm.launch",
        )
        ...
        # time.sleep(6)
        pub_trigger_process = start_process(
            "pub_trigger", "bash " + script_dir + "/../scripts/pub_trigger.sh"
        )
