          ...
          for (int indY = max(0, indYCenter - voxelNum_swarm_clearance_); indY <= min(indYCenter + voxelNum_swarm_clearance_, voxelNumY_ - 1); ++indY)
          {
            for (int indZ = max(0, indZCenter - voxelNum_swarm_clearance_); indZ <= min(indZCenter + voxelNum_swarm_clearance_, voxelNumZ_ - 1); ++indZ)
            {
              ...
              for (int k = 0; k < occPathNumByVoxel; k++)
              {
                ...
                // TODO: The end time of the trajectories 132-179 are 0.0 and hence the original condition never holds. If one fixes this the planner gets very unstable when the drone is close to obstacles or other drones. To fix this one has to do some more structural work and overthink the whole crash/no infeasible policy state. I created an github issue on their repo.
